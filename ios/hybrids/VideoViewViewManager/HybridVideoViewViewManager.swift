...
class HybridVideoViewViewManager: HybridVideoViewViewManagerSpec {
  ...
  let DEALOCATED_WARNING = "UnifiedPlayer: VideoComponentView is no longer available. It is likely that the view was deallocated."
    ...
    for pair in listeners where pair.eventName == eventName {
      if let callback = pair.callback as? T {
        do {
          ...
          print("[UnifiedPlayer] Error calling \(eventName) listener: \(error)")
        }
      } else {
        print("[UnifiedPlayer] Invalid callback type for \(eventName)")
      }
    }
  }
